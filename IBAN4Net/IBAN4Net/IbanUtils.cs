...
namespace IBAN4Net
{
    ...
    public static class IbanUtils
    {
        ...
        /// <summary>
        /// Test if two IBANs are from the same bank
        /// </summary>
        /// <param name="iban1">IBAN string 1</param>
        /// <param name="iban2">IBAN string 2</param>
        /// <param name="errorDescription">error details</param>
        /// <returns></returns>
        public static bool FromSameBank(string iban1, string iban2, out string errorDescription)
        {
            return _TestFromSameBank(iban1, iban2, TestSameBank.ignoreBranch, out errorDescription);

        }

        /// <summary>
        /// Test if two IBANs are from the same bank and branch
        /// </summary>
        /// <param name="iban1"></param>
        /// <param name="iban2"></param>
        /// <param name="errorDescription">error details</param>
        /// <returns></returns>
        public static bool FromSameBankAndBranch(string iban1, string iban2, out string errorDescription)
        {
            return _TestFromSameBank(iban1, iban2, TestSameBank.considerBranch, out errorDescription);
        }

        private static bool _TestFromSameBank(string iban1, string iban2, TestSameBank testBranch, out string errorDescription)
        {
            errorDescription = "";

            IbanFormatViolation result;
            bool isValid1 = IsValid(iban1, out result);
            bool isValid2 = IsValid(iban2, out result);

            if (!isValid1 || !isValid2)
            {
                errorDescription = !isValid1 ? "IBAN 1 is invalid. " : "";
                errorDescription += !isValid2 ? "IBAN 2 is invalid. " : "";
                return false;
            }


            var country1 = GetCountryCode(iban1);
            var country2 = GetCountryCode(iban2);
            var bank1 = GetBankCode(iban1);
            var bank2 = GetBankCode(iban2);
            var branch1 = GetBranchCode(iban1);
            var branch2 = GetBranchCode(iban2);

            if(testBranch == TestSameBank.ignoreBranch)
                return (country1 == country2 && bank1 == bank2);

            return (country1 == country2 && bank1 == bank2 && branch1 == branch2);
        }
